...
class FindPasswordController extends GetxController {
  ...
  Future<void> findPW(BuildContext context) async {
    ...
    if (email.isEmpty) {
      Get.snackbar('', '이메일을 입력하세요', snackPosition: SnackPosition.BOTTOM);
      return;
    }
    ...
    try {
      ...
      final resp = await http.post(
        Uri.parse('${ApiConstants.authApiBaseUrl}/reset-password'),
      ...
      if (resp.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('재설정 링크가 준비됐어요')),
        );
        _showResetLinkSheet(context, link, email);
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('요청 실패 (${resp.statusCode})')),
        );
